...
sold <- totalLoss(dframe,x,y,active,rank,level,sets)        ## compute loss value
iter <- pops <- 0
...
repeat {
	iter <- iter + 1
	y <- updateY(dframe, x, y, active, rank, level, sets, verbose = verbose)    ## this is the crucial part in the optimization
  ...
  aps <- sum(La.svd(crossprod(x,mis*z),0,0)$d)/ndim
	...
	if (iter == itermax) {
		warning("Maximum number of iterations reached. Increase itmax argument!")
		}
	
	if (smid > sold) {
		if (verbose == 0) warning("Increase of loss function during iteration! For a closer investigation set verbose = 1.")
	}
