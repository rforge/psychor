...
\Shorttitle{PAVA and Active Set \proglang{R}} %% a short title (if necessary)
...
Basically, there are two classes of algorithms to solve convex optimization problems: The first class are \emph{interior point methods} aim for complementary slackness while maintaining primal and dual feasbility. The second class are active set strategies where we distinguish between two variants:
\begin{itemize}
...
In \proglang{R} we can simply use the \code{optim()} function (e.g. with BFGS quasi-Newton) to minimize $f(G_I\xi)$ over $\xi$, and then set $\hat x=G_I\hat\xi$. This guarantees (if the optimum is found with sufficient precision) that the gradient at $\hat x$ is orthogonal to the indicator matrix $G_I$, and consequently that Lagrange multipliers can be computed. By making sure that $A_I$ has full row-rank, the Kuhn-Tucker vector is actually unique. Because of the way \code{optim()} is written it is also possible to tackle problems with non-differentiable loss functions, or even non-convex ones. But we are not responsible if this gets you into trouble.

To conclude, in this section we elaborated auxillary formulations of the active set problem. In the algorithmic implementation described in the next section we tackle optimization of problem $\mathcal{P}$ by defining and solving iteratively subproblems $\mathcal{P}_I$ (including the indicator matrix $G_I$). 
...
Active set descriptions for convex (quadratic) programming problems can be found in \citet[][Section 10.3]{Fletcher:1987} and \citet[][Section 16.4]{Nocedal+Wright:1999}. In general, based on a feasbile starting point, the active set strategy works as follows:
\begin{enumerate}
...
\end{enumerate}
These steps are repeated until we find a solution that is ``optimal enough''. In practice (and in our package), the zero-boundaries for the constraints and the dual feasiblity checking are relaxed in favor of a small value $\varepsilon$ which leads to $Ax \geq -\varepsilon$, and $\lambda \geq -\varepsilon$. 
...
\end{equation}
we find the optimum $\hat\xi$ by computing \emph{weighted medians} instead of weighted mid-ranges. Uniqueness problems, and the subdifferentials, will generally be much less smaller than in the case of $\ell_\infty$. 
...
The \pkg{isotone} package provides some pre-specified loss functions but allows also the user to define his own functions. The corresponding argument in the function \code{activeSet()} is \code{mySolver}. Each solver has extra arguments which we describe in this section. 
...
\begin{itemize}
\item \code{lsSolver}: Solves the least squares $\ell_2$ norm given in Equation \ref{eq:monreg}. 
\item \code{dSolver}: Minimizes the weighted absolute value $\ell_1$ norm as given in (\ref{eq:wabsval}).
\item \code{mSolver}: Solves the Chebyshev $\ell_\infty$ norm given in (\ref{eq:cheby}).
\item \code{pSolver}: Solves the quantile regression problem as given in (\ref{eq:quantile}).
\item \code{lfSolver}: Tackles a general least squares problem of the form $f(x)=(y-x)'W(y-x)$ where $W$ is a not necessarily positive semi-definite matrix of order $n$.   
\end{itemize}
...
\begin{itemize}
\item \code{sSolver}: Solves the negative Poisson log-likelihood with loss function $f(x)=\sum_{i=1}^n x_i-y_i\log(x_i)$.
\item \code{oSolver}: Minimizes the $\ell_p$ norm as given in (\ref{eq:loss}) with $m_i = 1\ \ \forall i$.
\item \code{aSolver}: \citet{Efron:1991} gives an asymmetric least squares formulation based on (\ref{eq:monreg}). The weights $b_w$ for $(y_i-x_i) \leq 0$ (extra argument \code{bw}) and $a_w$ for $(y_i-x_i) > 0$ (extra argument \code{aw}) are introduced. Consequently, $h_i(y_i,x_i) = (y_i-x_i)^2b_w$ for $(y_i-x_i)\leq 0$ and $h_i(x_i,y_i) = (y_i-x_i)^2a_w$ for $(y_i-x_i) > 0$. 
\item \code{eSolver}: Minimizes the $\ell_1$ approximation for $f(x)=\sum_{i=1}^n w_i\sqrt{(y_i-x_i)^2+\varepsilon}$. Correspondingly, the additional extra argument is \code{eps}. 
\item \code{hSolver}: Solves the Huber loss-function \citep{Huber:1981} with extra argument \code{eps}. 
\[
h_i(y_i,x_i)= \left\{
\begin{array}{ll}
...
\right.
\]
\item \code{iSolver}: Minimizes the soft insensitive loss function (SILF) defined in \citet{Chu+Keerthi+Ong:2004} with the two extra parameters $\varepsilon$ (argument \code{eps}) and $\beta$ (argument \code{beta}). We have to distinguish as follows:
\[
h_i(y_i,x_i)= \left\{
\begin{array}{ll}
...
\label{sec:packpava}
Table \ref{tab:pava} gives an overview of available PAVA implementations in \proglang{R}. It quotes the package name, the corresponding function, whether this function is externally accessible, a brief description, and in which language the core computations are implemented. 
\begin{table}
\begin{center}
...
\hline
\pkg{stats} & \code{isoreg()} & external & isotonic regression &  C \\
\pkg{monreg} & \code{monreg()} & external & monotonic regression & C \\
\pkg{fdrtool} & \code{monoreg()} & external & monotonic regression, weights & C \\
\pkg{Cir} & \code{pava()}, \code{cir.pava()} & external & isotonic regression, weights & R \\
\pkg{Iso} & \code{pava()} & external & isotonic regression, weights & FORTRAN \\
\pkg{clue} & \code{pava()} & internal & isotonic regression (mean/median) & R \\
\pkg{logcondens} & \code{isoMean()} & external & isotonic regression, weights & R \\ 
\pkg{sandwich} & \code{pava.blocks()} & internal & isotonic regression, weights & R\\
\pkg{intcox} & \code{intcox.pavaC()} & internal & isotonic regression, weights & C \\
\pkg{SAGx} & \code{pava.fdr()} & external & FDR using isotonic regression & R \\
\pkg{smacof} & \code{pavasmacof()} & internal & nonmetric MDS, ties & R\\
\hline 
\end{tabular}
...
coordination of visual, proprioceptive and vestibular components of balance to maintain standing posture. These equilibrium scores represent the angular difference between the subject's calculated anterior/posterior centre of gravity displacements and the theoretical maximum of $12.5^{\circ}$. A score of 100 theoretically indicates no anterior/posterior excursion. A score of 0 indicates a fall of the subject. Thus, the higher the score, the more able a person is to maintain the balance. 
...
\begin{itemize}
  \item \code{z}: Vector containing observed predictor values $z$. The response values $y$ are passed to the solver function (see below).
...
The predictor values $z$ define the order for the response $y$. The variable \code{btota} defines the pairwise isotonicity matrix which needs to have 2 columns and as many rows as unique isotonicity constraints.
\begin{Schunk}
\begin{Sinput}
> Atot
\end{Sinput}
...
> fit.ls2 <- activeSet(z, Atot, fSolver, fobj = function(x) sum(w1*(x-y)^2), 
...
\begin{Schunk}
\begin{Sinput}
> fit.qua <- activeSet(z, Atot, pSolver, y = y, weights = wvec, aw = 0.3, 
+     bw = 0.7)
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
> fit.eps <- activeSet(z, Atot, eSolver, y = y, weights = w1, eps = 1e-04)
...
> fit.svm <- activeSet(z, Atot, iSolver, y = y, weights = w1, beta = 0.8, 
+     eps = 0.2)
\end{Sinput}
\end{Schunk}
...
\begin{Schunk}
\begin{Sinput}
> Aloop <- matrix(c(1, 2, 3, 3, 4, 5, 6, 6, 7, 8, 3, 3, 4, 5, 6, 6, 7, 8, 9, 9), 
+     10, 2)
> Aloop
\end{Sinput}
\begin{Soutput}
...
In Section \ref{sec:gpavath} we mentioned that PAVA can be considered as a dual active set method. Using the simulated data again, we show that LS PAVA and LS active set lead to the same results. Consequently, the mean squared error becomes 0. 
