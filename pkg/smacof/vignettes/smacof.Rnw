...
one can compute proximities such as correlations or various kind of distance measures. The \pkg{smacof} package always
requires dissimilarities as input. Having similarities such as correlations, the \code{sim2diss} helper function offers possibilities for corresponding transformations. 

The next dataset is based on ratings of \Sexpr{nrow(RockHard)} records in a German Heavy Metal magazine called Rock Hard, collected in 2013. For the moment we are only interested in the judges who rated each record on a scale from 0 (super bad) 
to 10 (incredibly awesome), .5 ratings were allowed. We use an MDS to explore which judges have a similar taste. 
...
As descriptive measures a stability measure \citep{Heiser+Meulman:1983} can be computed as follows:
\[
...
\begin{figure}
\begin{center}  
<<jack-plot1, echo=FALSE, fig.width = 8, fig.height = 8, dev='postscript'>>=
<<jack-plot>>
@
\end{center}
