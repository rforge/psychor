%\VignetteIndexEntry{An Introduction to the homals package}
...
\subsection{Nonlinear (categorical) principal component analysis}
...
\subsection{Categorical Principal Component Analysis}
