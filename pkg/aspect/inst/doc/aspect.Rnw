...
\label{sec:sem}
In the case of categorical (ordinal) data, SEM software such as \proglang{EQS} \citep{Bentler:1995} and \proglang{LISREL} \citep{Joreskog+Sorbom:2005} computes the polychoric correlation matrix based on the indicators. \proglang{EQS} uses the \citet{Lee+Poon+Bentler:1992} approach whereas \proglang{LISREL} provides the \proglang{PRELIS} module \citep{Joreskog:2005}. Note that polychoric correlations require an underlying normality assumption.
...
\end{itemize}
Note that this dataset, which is included in the package, provides also the amount of alcohol comsumption. For illustrational issues we only use marijuana (\code{POT\_T1}, ..., \code{POT\_T4}) and cigarettes (\code{CIG\_T1}, ..., \code{CIG\_T4}).
